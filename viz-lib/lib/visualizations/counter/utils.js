...
function numberFormat(value, decimalPoints, decimalDelimiter, thousandsDelimiter) {
    ...
    const locale = numeral_1.default.localeData();
    ...
    const result = (0, numeral_1.default)(value).format(formatString);
...
function formatValue(value, { stringPrefix, stringSuffix, stringDecimal, stringDecChar, stringThouSep }) {
    if ((0, lodash_1.isNumber)(value)) {
        ...
        return (0, lodash_1.toString)(stringPrefix) + value + (0, lodash_1.toString)(stringSuffix);
    }
    return (0, lodash_1.toString)(value);
}
function formatTooltip(value, formatString) {
    if ((0, lodash_1.isNumber)(value)) {
        return (0, numeral_1.default)(value).format(formatString);
    }
    return (0, lodash_1.toString)(value);
}
function getCounterData(rows, options, visualizationName) {
    ...
    if (rowsCount > 0 || options.countRow) {
        ...
        if (targetColName) {
            ...
            // @ts-expect-error ts-migrate(2339) FIXME: Property 'counterValue' does not exist on type '{}... Remove this comment to see the full error message
            if (Number.isFinite(result.counterValue) && (0, lodash_1.isFinite)(result.targetValue)) {
                // @ts-expect-error ts-migrate(2339) FIXME: Property 'counterValue' does not exist on type '{}... Remove this comment to see the full error message
        ...
        if (options.formatTargetValue) {
            ...
            // @ts-expect-error ts-migrate(2339) FIXME: Property 'targetValue' does not exist on type '{}'... Remove this comment to see the full error message
            if ((0, lodash_1.isFinite)(result.targetValue)) {
                // @ts-expect-error ts-migrate(2339) FIXME: Property 'targetValue' does not exist on type '{}'... Remove this comment to see the full error message
                result.targetValue = (0, numeral_1.default)(result.targetValue).format("0[.]00[0]");
            }
        }
    }
    return result;
...
function isValueNumber(rows, options) {
    ...
    if (rowsCount > 0) {
        ...
        const counterColName = options.counterColName;
        if (counterColName) {
            return (0, lodash_1.isNumber)(rows[rowNumber][counterColName]);
        }
    }
    return false;
}
