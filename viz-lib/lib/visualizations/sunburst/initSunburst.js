...
function buildNodesFromHierarchyData(data) {
    const grouped = (0, lodash_1.groupBy)(data, "sequence");
    return (0, lodash_1.map)(grouped, value => {
        const sorted = (0, lodash_1.sortBy)(value, "stage");
        return {
            size: value[0].value || 0,
            sequence: value[0].sequence,
            nodes: (0, lodash_1.map)(sorted, i => i.node),
        };
    });
}
function buildNodesFromTableData(data) {
    ...
    return (0, lodash_1.map)(data, (row, sequence) => ({
        size: row.value || 0,
        sequence,
        nodes: (0, lodash_1.compact)((0, lodash_1.map)(dataKeys, key => row[key])),
    }));
}
function isDataInHierarchyFormat(data) {
    const firstRow = (0, lodash_1.first)(data);
    return (0, lodash_1.every)(["sequence", "stage", "node", "value"], field => (0, lodash_1.has)(firstRow, field));
}
function buildHierarchy(data) {
    ...
    data.forEach((d) => {
        ...
        for (let j = 0; j < nodes.length; j += 1) {
            ...
            let childNode = (0, lodash_1.find)(children, child => child.name === nodeName);
            if (isLeaf && childNode) {
