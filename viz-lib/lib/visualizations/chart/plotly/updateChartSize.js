...
function placeLegendNextToPlot(plotlyElement, layout) {
    const transformName = (0, lodash_1.find)(["transform", "WebkitTransform", "MozTransform", "MsTransform", "OTransform"], prop => prop in plotlyElement.style);
    layout.legend = (0, lodash_1.extend)({}, layout.legend, {
        orientation: "v",
    ...
    return [(0, lodash_1.pick)(layout, ["width", "height", "legend"]), null]; // no further updates
}
function placeLegendBelowPlot(plotlyElement, layout) {
    ...
    return [
        (0, lodash_1.pick)(layout, ["width", "height", "legend"]),
        () => {
            ...
            if (legend) {
                ...
                return [(0, lodash_1.pick)(layout, ["height"]), null]; // no further updates
            }
        },
    ];
}
...
function updateChartSize(plotlyElement, layout, options) {
    ...
    if (options.legend.enabled) {
        ...
        return [(0, lodash_1.pick)(layout, ["width", "height"]), null]; // no further updates
    }
}
