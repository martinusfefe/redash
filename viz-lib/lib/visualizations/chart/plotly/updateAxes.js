...
function calculateAxisRange(range, min, max) {
    return [(0, lodash_1.isNumber)(min) ? min : range[0], (0, lodash_1.isNumber)(max) ? max : range[1]];
}
...
function updateAxes(plotlyElement, seriesList, layout, options) {
    const updates = {};
    if ((0, lodash_1.isObject)(layout.yaxis)) {
    ...
    if ((0, lodash_1.isObject)(layout.yaxis2)) {
    ...
    return [
        ...
        () => {
            ...
            if (options.swappedAxes) {
                ...
                if ((0, lodash_1.isObject)(xaxis) && (0, lodash_1.isObject)(yaxis)) {
                    // @ts-expect-error ts-migrate(2339) FIXME: Property 'xaxis' does not exist on type '{}'.
                    updates.xaxis = yaxis;
                    // @ts-expect-error ts-migrate(2339) FIXME: Property 'yaxis' does not exist on type '{}'.
                    updates.yaxis = xaxis;
                }
                if ((0, lodash_1.isObject)(yaxis2)) {
                    // @ts-expect-error ts-migrate(2339) FIXME: Property 'yaxis2' does not exist on type '{}'.
                    updates.yaxis2 = null;
                }
            }
            // Align Y axes
            if (options.alignYAxesAtZero && (0, lodash_1.isObject)(layout.yaxis) && (0, lodash_1.isObject)(layout.yaxis2)) {
                // @ts-expect-error ts-migrate(2339) FIXME: Property 'yaxis' does not exist on type '{}'.
