...
function prepareCustomChartData(series) {
    const x = [];
    const ys = {};
    (0, lodash_1.each)(series, ({ name, data }) => {
        // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        ys[name] = [];
        (0, lodash_1.each)(data, point => {
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2-3 arguments, but got 1.
            x.push((0, utils_1.normalizeValue)(point.x));
            // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            ys[name].push((0, utils_1.normalizeValue)(point.y));
        });
    });
    return { x, ys };
}
function createCustomChartRenderer(code, logErrorsToConsole = false) {
