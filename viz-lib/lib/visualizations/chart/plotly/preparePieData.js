...
function preparePieData(seriesList, options) {
  ...
  if (typeof seriesList[0] !== 'undefined' && _ColorPalette.ColorPaletteTypes[options.color_scheme] === 'continuous') {
    ...
    var colorIndices = _d.default.range(uniqueXValues.length).map(function (i) {
      return Math.round(step * i);
    });
    // @ts-expect-error ts-migrate(2339) FIXME: Property 'scale' does not exist on type 'typeof im... Remove this comment to see the full error message
    getDefaultColor = _d.default.scale.ordinal().domain(uniqueXValues) // Set domain as the unique x-values
    .range(colorIndices.map(index => palette[index]));
  } else {
    // @ts-expect-error ts-migrate(2339) FIXME: Property 'scale' does not exist on type 'typeof im... Remove this comment to see the full error message
    getDefaultColor = _d.default.scale.ordinal().domain([]).range(palette);
  }
  ;
