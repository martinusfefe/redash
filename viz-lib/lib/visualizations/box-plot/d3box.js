...
function box() {
  ...
  function box(g) {
    g.each(function (d, i) {
      ...
      // @ts-expect-error ts-migrate(2683) FIXME: 'this' implicitly has type 'any' because it does n... Remove this comment to see the full error message
      .domain(domain && domain.call(this, d, i) || [min, max]).range([height, 0]);
      ...
      center.enter().insert("line", "rect").attr("class", "center").attr("x1", width / 2).attr("y1", d => x0(d[0])).attr("x2", width / 2).attr("y2", d => x0(d[1])).style("opacity", 1e-6).transition().duration(duration).style("opacity", 1).attr("y1", d => x1(d[0])).attr("y2", d => x1(d[1]));
      center.transition().duration(duration).style("opacity", 1).attr("y1", d => x1(d[0])).attr("y2", d => x1(d[1]));
      center.exit().transition().duration(duration).style("opacity", 1e-6)
      // @ts-expect-error ts-migrate(2571) FIXME: Object is of type 'unknown'.
      .attr("y1", d => x1(d[0]))
      // @ts-expect-error ts-migrate(2571) FIXME: Object is of type 'unknown'.
      .attr("y2", d => x1(d[1])).remove();

      // Update innerquartile box.
      var box = g.selectAll("rect.box").data([quartileData]);
      box.enter().append("rect").attr("class", "box").attr("x", 0).attr("y", d => x0(d[2])).attr("width", width).attr("height", d => x0(d[0]) - x0(d[2])).transition().duration(duration).attr("y", d => x1(d[2])).attr("height", d => x1(d[0]) - x1(d[2]));
      box.transition().duration(duration).attr("y", d => x1(d[2])).attr("height", d => x1(d[0]) - x1(d[2]));
      box.exit().remove();
      ...
      outlier.enter().insert("circle", "text").attr("class", "outlier").attr("r", 5).attr("cx", width / 2).attr("cy", i => x0(d[i])).style("opacity", 1e-6).transition().duration(duration).attr("cy", i => x1(d[i])).style("opacity", 1);
      outlier.transition().duration(duration).attr("cy", i => x1(d[i])).style("opacity", 1);
      outlier.exit().transition().duration(duration)
      // @ts-expect-error ts-migrate(2538) FIXME: Type 'unknown' cannot be used as an index type.
      .attr("cy", i => x1(d[i])).style("opacity", 1e-6).remove();
      ...
      boxTick.enter().append("text").attr("class", "box").attr("dy", ".3em").attr("dx", (d, i) => i & 1 ? 6 : -6).attr("x", (d, i) => i & 1 ? width : 0).attr("y", x0).attr("text-anchor", (d, i) => i & 1 ? "start" : "end").text(format).transition().duration(duration).attr("y", x1);
