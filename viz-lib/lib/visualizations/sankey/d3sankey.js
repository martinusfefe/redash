...
function Sankey() {
    ...
    function computeNodeValues() {
        nodes.forEach(node => {
            node.value = Math.max(d3_1.default.sum(node.sourceLinks, value), d3_1.default.sum(node.targetLinks, value));
        });
    }
    ...
    function computeNodeBreadths() {
        ...
        moveSinksRight(x);
        x = Math.max(d3_1.default.max(nodes, n => n.x), 2); // get new maximum x value (min 2)
    ...
    function computeNodeDepths(iterations) {
        const nodesByBreadth = d3_1.default
            ...
            .nest()
            .key((d) => d.x)
            .sortKeys(d3_1.default.ascending)
            .entries(nodes)
            .map((d) => d.values);
        function initializeNodeDepth() {
            // @ts-expect-error ts-migrate(2571) FIXME: Object is of type 'unknown'.
            const ky = d3_1.default.min(nodesByBreadth, n => (size[1] - (n.length - 1) * nodePadding) / d3_1.default.sum(n, value));
            ...
            nodesByBreadth.forEach((n) => {
                n.forEach((node) => {
                    if (node.targetLinks.length) {
                        const y = d3_1.default.sum(node.targetLinks, weightedSource) / d3_1.default.sum(node.targetLinks, value);
        ...
        function relaxRightToLeft(alpha) {
            nodesByBreadth
                ...
                nodes.forEach((node) => {
                    if (node.sourceLinks.length) {
                        const y = d3_1.default.sum(node.sourceLinks, weightedTarget) / d3_1.default.sum(node.sourceLinks, value);
                        node.y += (y - center(node)) * alpha;
                    }
                });
            });
    ...
    sankey.link = function () {
        ...
        function link(d) {
            ...
            const x1 = d.target.x;
            const xi = d3_1.default.interpolateNumber(x0, x1);
